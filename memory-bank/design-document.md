...
- 老师名、学校别名用规则 + 学校别名表。
- `research_areas` / `research_topics` **必须经 LLM 审阅**，不用关键词表决定「这是不是方向」。申请材料（CV、成绩单、论文列表）不得进方向；「CV」不是 computer vision。无密钥时使用已审缓存，不得用更长的正则去补。
...
- 托管：GitHub Pages 静态站。
- 主表默认时间窗：18 个月。
- 姓名/学校/联系方式仍用规则；研究方向由 LLM 审阅（可缓存，测试注入假模型）。
